...
通过 CLI 读取学城文档的各种信息和内容、创建学城文档、查看文档的划词评论，查看最近访问、编辑、收到的学城文档列表。认证自动处理。直接调用线上接口，响应速度更快。
...
### 给了 km.sankuai.com 链接或者文档ID，读取/查看/总结学城文档
...
通用选项：`--mis <mis>` 指定用户，`--raw` 输出 JSON 到 stdout，`--clear-cache` 清除认证缓存。
...
用户要求时明确说明"当前暂不支持"。替代方案：先用 `getMarkdown` 读取内容，在本地生成修改建议。
