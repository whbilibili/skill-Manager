...
当用户意图命中路由表中的特定场景时，在执行路由前输出关联提醒。

| 路由目标 | 触发条件 | 提醒时机 |
|----------|----------|----------|
| 考勤 Skill（请假/休假/调休） | 用户意图为请假、休假、调休，准备路由到 attendance-approval-helper | 路由前追加提醒，然后正常路由 |

**话术模板：**

```
正在为你转到考勤系统发起请假申请。
💡 提醒：如果假期较长（≥2天），请假后可以跟我说"设置审批授权"，我帮你配好代办人，避免审批积压。
```
...
- 提醒后正常执行路由，不阻断用户操作
- 用户无需回应提醒即可继续
- 仅在路由到请假/休假/调休相关流程时触发，补卡/申诉等不触发
- 如果用户当前已有 ACTIVE 状态的审批授权（通过 `/v1.1/auth/list` 查询 `authStatus: 'EFFECTIVE'`），跳过提醒
