> **说明**：Step 2 采用 **A+B 双轨制**——内置控件速查表覆盖常见场景（零依赖）；遇到复杂控件时自动安装 [快搭助手](https://friday.sankuai.com/skills/skill-detail?id=32795) skill 并引用其完整格式规范。详见 Step 2。
...
**B 方案**：自动安装并引用 [快搭助手](https://friday.sankuai.com/skills/skill-detail?id=32795) skill 的 `references/fill-form.md` 获取完整控件格式规范。
