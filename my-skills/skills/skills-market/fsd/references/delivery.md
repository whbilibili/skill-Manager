...
- **`--deploy-template`**：与 **`--lite-set-template`** 共用语义——非 liteSet 时校验 **`POST /api/qa/v1/job/getTemplates?env=`**（test / cargo / staging 由当前运行环境决定）；liteSet 时校验 **getLiteSetTemplates**（`env=prod&source=0`、`sourceId=applyProgramId`）。未传模板时取接口返回的**各服务列表首项**（首项无效则拒绝触发）。传入名称/id 且**非 liteSet** 时：每服务为 **唯一命中**、**零命中**，或「部分命中、部分零命中」（命中多项的服务仍立即失败）；**无一服务命中**则立即失败；**既有命中又有未命中**时 `fsd delivery trigger` / `trigger-retry`（非 `--dry-run`）在 stderr 列已匹配/未匹配并提示 **`y/N`**，仅 `y`/`yes` 或 **`--yes-partial-deploy-template`** 才只对已命中应用继续；否认、EOF、非 TTY 且无该开关则整单失败；**不写模板**不适用此交互。**`trigger-retry --dry-run`**：不询问 stdin，按「仅已匹配」收窄预览 JSON。**liteSet**：**`liteSetExplicitFilterJobNames`** 仍须全体同时满足。**`bdsMode === 'npm'`** 时见 [deploy.md · npm 与 --deploy-template](deploy.md#npm-bds-deploy-template)。说明链：**`FSD_JOB_DEPLOY_TEMPLATE_DOC_URL`**（与 liteSet 策略文档同域）。
