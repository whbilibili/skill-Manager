...
- `grant`、`modify`、`revoke`、`clear-perm` 会先创建权限备份文档，再执行批量操作。
...
**输出**：成功/失败统计、备份文档链接；有失败时附失败清单链接。

---

### modify — 批量改权

批量把目标对象**已有**的权限改为新权限。若目标对象对某篇文档没有权限记录，该文档会计入 `skipped`，不会新增权限。参数与 `grant` 完全一致（同样会创建备份文档）。
...
**输出**：成功/跳过/失败统计、备份文档链接；有失败时附失败清单链接。

---

### revoke — 批量移权

批量删除目标对象的显式权限。目标参数与 `grant` 相同，但**不需要 `--perm`**。会创建备份文档。
...
⚠️ **高风险操作**：清除指定范围内所有文档的**全部显式权限**（仅保留所有者和空间管理员），并关闭链接分享权限。执行前必须向用户明确说明后果，并获得确认。会自动创建备份文档。
