...
| AI 生成/辅助的代码（任何技术栈） | `ai-code-trust` | 读取 `~/.catpaw/skills/ai-code-trust/SKILL.md`，按其"被引用接口"约定执行信任检查 |
...
### Step 4.5：AI 代码信任检查（引用 `ai-code-trust`）

> **本步骤仅在检测到 AI 产出代码时执行。** 如果代码完全由人工编写，跳过此步。

#### 触发判断

检查以下条件，满足任一即触发：

1. commit message 包含 AI 工具标识（Copilot、Cursor、Claude、ChatGPT、Codeium 等）
2. 用户明确说明代码是 AI 生成或 AI 辅助完成的
3. 单次提交新增代码 > 200 行，且无逐行 review 记录
4. 用户主动要求信任检查

#### 执行方式

读取 `~/.catpaw/skills/ai-code-trust/SKILL.md`，按其**被引用接口**中的约定执行：

- **默认使用嵌入模式**：执行 ai-code-trust 的 Step 1（决策审计）+ Step 2（脆弱性扫描），将结果嵌入本报告的 `🔍 AI 代码信任检查` 章节。
- **用户要求完整验收时切换完整模式**：执行全部四步（决策审计→脆弱性扫描→边界爆破→可修复性评估），将完整信任报告作为附录。

#### 结果纳入最终结论

信任等级与本 Skill 的 P 级映射：
- 🔴 信任等级 → 等同 P1（强烈建议修复），影响最终结论
- 🟡 信任等级 → 等同 P2（应修复）
- 🟢 信任等级 → 不影响最终结论

#### 降级

如果 `~/.catpaw/skills/ai-code-trust/SKILL.md` 不存在，输出：
```
⚠️ ai-code-trust Skill 未安装，跳过 AI 代码信任检查。建议安装以获得 AI 产出代码的信任校准能力。
```
...
> 仅在 Step 4.5 触发时输出本章节。未触发时整节省略。

**信任等级**：🟢 可信 / 🟡 有条件可信 / 🔴 需要修复
**检查模式**：嵌入模式（Step 1+2）/ 完整模式（Step 1-4）

| 检查步骤 | 状态 | 核心发现 |
|----------|------|----------|
| 决策审计 | 🟢/🟡/🔴 | [一句话总结] |
| 脆弱性扫描 | 🟢/🟡/🔴 | [发现 X 个问题] |
| 边界爆破（完整模式） | 🟢/🟡/🔴 | [X 个用例通过/失败] |
| 可修复性（完整模式） | 🟢/🟡/🔴 | [一句话总结] |

**信任检查发现的问题**（已纳入上方对应 P 级）：
[列出信任检查中发现的关键问题，标注已映射到的 P 级]
...
| P2（规范） | [n] | [📝 应修复 / ✅ 清零] |
| P3（建议） | [n] | [💡 可选 / ✅ 清零] |
| AI 信任检查 | [🟢/🟡/🔴 或 "未触发"] | [信任等级说明] |

**最终结论**：
- 🚫 **阻塞合并**：存在 P0 问题，必须修复后重新 review
- ⚠️ **有条件通过**：无 P0，有 P1/P2（含信任检查映射的问题），作者确认处理方案后可合并
- ✅ **可以合并**：无 P0/P1，P2/P3 已知晓，信任检查通过或未触发，可直接合并
```
