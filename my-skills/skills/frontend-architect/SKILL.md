...
1. **跳过技术栈决策**：不再自行选型，直接从 Tech Spec 区块 2（技术选型决策表）读取前端相关选型
2. **直接采用状态模型**：Tech Spec 区块 3（前端版为「状态模型设计」）直接作为 feature-list.json 中 `contracts.state_management` 的权威来源
3. **基于模块边界拆解**：Tech Spec 区块 5 的前端模块划分直接决定 Task 的分组方式
4. **引用 API 消费清单**：Tech Spec 区块 6 的 API 清单直接作为 `contracts.api_consumption` 的路径和方法来源
5. **继承非功能性约束**：Tech Spec 区块 7 的前端性能指标（首屏、Bundle Size、CLS）直接写入 `ARCHITECTURE.md`
6. **设计系统仍然执行**：即使有 Tech Spec，Step 1-4（设计系统生成）仍然必须执行（Tech Spec 不覆盖视觉设计）

**输出变更**：
- Step 1 的选型理由改为输出 `✅ 基于已审批的 Tech Spec (v{版本号}) 进行任务拆解`
- 项目目录树摘要仍然输出（基于 Tech Spec 选型推导）
- 全局状态边界直接从 Tech Spec 区块 3 的「Server State vs Client State 边界」表格获取
- `progress.txt` 的 `[Key Decisions]` 中注明：`技术选型来源：docs/design-docs/tech-spec.md (approved)`

> ⚠️ **一致性约束**：Tech Spec 驱动模式下，feature-list.json 中的任何 `contracts` 字段不得与 Tech Spec 产生矛盾。如果拆解过程中发现 Tech Spec 有遗漏（如某个页面的 API 消费未在区块 6 中列出），必须在 `progress.txt` 的 `[Blockers & Solutions]` 中标注，并建议用户回到 tech-spec-architect 补充。

---

### Step 1：技术栈决策与目录规约

> 💡 **注意**：如果 Step 0.5 已进入 Tech Spec 驱动模式，本步骤跳过选型决策和全局状态边界（从 Tech Spec 继承），仅执行目录树摘要和设计系统生成。
