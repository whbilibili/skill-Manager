...
description: "Manage CatPaw Desk settings, custom commands, MCP servers, app lifecycle, logs, sessions, and authentication via CLI, plus show native system notifications. Use when the user asks to: change preferences/toggle features/fonts/shortcuts/any app setting, switch sandbox or confirm-before-delete policy, create/edit/delete custom (slash) commands, add/remove/enable/disable MCP servers, show a notification, check/update version, restart the app, view application or conversation logs, list sessions or get current session/conversationId, check session or safe house status, toggle prevent sleep, get login info, get/exchange SSO token."
---
...
All commands: `catdesk settings <action> [options]`

### List all settings (always do this first)

```bash
catdesk settings list
```
...
```bash
catdesk settings get --key <settingKey>
```

### Set a single setting

```bash
catdesk settings set --key <settingKey> --value <newValue>
```

### Batch set multiple settings

```bash
catdesk settings set --json '{"key1":"value1","key2":"value2"}'
```

### Reset a setting / all settings

```bash
catdesk settings reset --key <settingKey>
catdesk settings reset --all
```
...
All commands: `catdesk commands <action> [options]`

### List all commands

```bash
catdesk commands list
```
...
```bash
catdesk commands get --name <commandName>
```
...
```bash
catdesk commands create --name <commandName> [--content "# My Command\n\nInstructions..."]
```
...
```bash
catdesk commands update --name <commandName> --content "new markdown content"
```

### Delete a command

```bash
catdesk commands delete --name <commandName>
```
...
All commands: `catdesk mcp <action> [options]`
...
```bash
catdesk mcp get --name <serverName>
```

### Add or update an MCP server

```bash
catdesk mcp add --name <serverName> --json '<configJson>'
```
...
**Stdio** (local process):
```bash
catdesk mcp add --name my-tool --json '{"command":"npx","args":["-y","some-mcp"],"env":{"API_KEY":"xxx"}}'
```

**SSE** (Server-Sent Events):
```bash
catdesk mcp add --name my-sse --json '{"type":"sse","url":"http://localhost:3000/sse"}'
```

**Streamable HTTP**:
```bash
catdesk mcp add --name my-http --json '{"type":"streamable-http","url":"http://example.com/mcp","headers":{"Authorization":"Bearer xxx"}}'
```
...
```bash
catdesk mcp remove --name <serverName>
```

### Enable/disable an MCP server

```bash
catdesk mcp toggle --name <serverName> --enable
catdesk mcp toggle --name <serverName> --disable
```
...
```bash
catdesk notify -m "message" [-t "title"] [--type info|warning|error]
```
...
All commands: `catdesk app <sub-command>`
...
All commands: `catdesk session <sub-command>`
...
All commands: `catdesk log <sub-command> [options]`

### Show main process log

```bash
catdesk log main
catdesk log main -n 200    # last 200 lines (default: 100)
```

### Show renderer process log

```bash
catdesk log renderer
catdesk log renderer -n 50
```
...
```bash
catdesk log conversation --id <conversationId>
catdesk log conversation --id <conversationId> -n 50   # last 50 lines
```
...
All logs are stored in `~/.catdesk/logs/`:
...
All commands: `catdesk auth <sub-command> [options]`
...
```bash
catdesk auth exchange --target-client-id <clientId>
```
...
- **Logs** are stored in `~/.catdesk/logs/`. Conversation logs are auto-cleaned on app startup (keeps max 50 files or 7 days). Main/renderer/agent logs have size-based rotation.
